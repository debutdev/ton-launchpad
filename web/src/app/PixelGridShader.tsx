...
function evalShape(nx: number, ny: number, t: number, s: AppState): number {
  ...
  if (shape === 'wave') {
    ...
    const nuvX = nx * 0.1 * frequency * 80;
    const nuvY = ny * 0.1 * frequency * 80;
    v = fbm(nuvX - t, nuvY + t);
    ...
    const r =
      Math.sin(Math.pow(dist * 2, 2 + amplitude * 0.7) * rings * PI - frequency * t) *
        0.5 +
      0.2;
    v = r * smoothstep(0.5, 2, dist);
  } else if (shape === 'swirl') {
    const l = Math.sqrt(nx * nx + ny * ny);
    const angle = rings * Math.atan2(ny, nx) + frequency * t;
  ...
  if (cursor.active) {
    ...
    const ripple =
      Math.sin(scaledDist * 30 * s.cursorScale - t * 6 * s.cursorScale) * 0.5 + 0.5;
